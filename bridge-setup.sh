#!/bin/bash
# create bridge
sudo ip link add name br) type bridge &> /dev/null
...
sudo ip netns exec peach ip link set dev peachnet up &> /dev/null
sudo ip netns exec peach ip link set dev lo up &> /dev/null
sudo ip netns exec bowser ip link set dev bowser &> /dev/null
